...
#include "App.h"
#include "..\src\services\modbus\Service_atModbus_RTU.h"
 ...
 */
class App_MB_RTU : public Application
{
public:
  App_MB_RTU();
 	~App_MB_RTU();
...
***********/
  ...
  for( int count = GENERAL_REGISTER_RW_MODBUS_RTU_ID; count <= GENERAL_REGISTER_RW_WIFI_SUBNET_A; count++){
     mb_RTU.addHreg(count,count);
     }

  for( int count = GENERAL_REGISTER_RW_WIFI_SUBNET_B; count <= GENERAL_REGISTER_R_HUMIDITY; count++){
     mb_RTU.addHreg(count,count);
     }
 /***********
 ---------- Init SNM Register ------------
 ...
 */
void  App_MB_RTU::App_MB_Restart()
{
 ...
 */
void  App_MB_RTU::App_MB_Execute()
{	    
...
void  App_MB_RTU::App_MB_Suspend(){}
void  App_MB_RTU::App_MB_Resume(){}	  
void  App_MB_RTU::App_MB_End(){}
